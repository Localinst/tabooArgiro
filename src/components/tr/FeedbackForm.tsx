...
const FeedbackForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSuccess(false);
    try {
      const content = email ? `E-posta: ${email}\n${message}` : message;
      ...
      if (res.ok) {
        ...
        setError('Geri bildirim gönderilirken hata oluştu. Lütfen tekrar deneyin.');
      }
    } catch (err) {
      setError('Ağ hatası.');
    } finally {
  ...
  return (
    <Card className="w-full max-w-md mx-auto mt-8 bg-white/60 backdrop-blur-sm">
      <CardHeader>
        <CardTitle>Geri Bildirim Bırakın</CardTitle>
      </CardHeader>
      <CardContent>
        {success ? (
          <div className="text-green-600 text-center py-4">Geri bildiriminiz için teşekkürler!</div>
        ) : (
          <form onSubmit={handleSubmit}>
            <div className="space-y-4">
              <div>
                <Label htmlFor="email">E-posta (isteğe bağlı, cevap için)</Label>
              ...
              <div>
                ...
                <Textarea 
                  ...
                  maxLength={100}
                />
                <div className="text-sm text-gray-500 mt-1">{message.length}/100 karakter</div>
              </div>
